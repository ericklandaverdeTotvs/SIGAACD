...
User Function IMX4601(nApp)
	...
	Do While lContinuar
		...
		ElseIf nApp == 3
			@00, 00 vtSay "= Orden de surtido / Picking ="
		End If
...
User Function IMX4601B(nApp, aDatos)
	...
	Do While lContinuar
		...
		//@07, 00 vtSay "6) Concluir documento"
		...
		Do Case
			...
			//Case cOpcion == "6" //Concluir documento
				//lContinuar := .F.
				//U_IMX4601L(nApp, aDatos)
			Otherwise
				vtAlert(Chr(13) + Chr(10) + "Opcion" + Chr(13) + Chr(10) + "invalida", "SIGAACD", .T.)
		End
		vtClear()
...
User Function IMX4601D(nApp, aDatos, nOpcion, lConfirma)
	Local	lContinuar	:=	.T.
	Private	nZ3_ITEM	:=	0
	Private	cZ3_COD		:=	Space(30)
	Private	cZ3_DESC	:=	Space(25)
	Private cZ3_LOT		:=	Space(30)
	...
	Do While lContinuar
		...
		If vtLastKey() == 13 //Enter
			U_IMX4601F(nApp, aDatos, nOpcion, lConfirma)
			cZ3_COD		:=	Space(30)
			cZ3_DESC	:=	Space(25)
			cZ3_LOT		:=	Space(30)
			cZ3_QTY		:=	Space(11)
...
static function actualizar(nApp, nOpcion, cBuffer, aDatos)
	Local	lResult	:=	U_IMX4601E(nApp, nOpcion, cBuffer, aDatos)
	vtGetRefresh(cZ3_COD)
	vtGetRefresh(cZ3_DESC)
	vtGetRefresh(cZ3_LOT)
	vtGetRefresh(cZ3_QTY)
	If nOpcion == 2 .And. U_IMX4600H(cZ3_COD) .And. empty(cZ3_LOT)
		vtAlert("El producto controla lote.", "SIGAACD", .T.)
		lResult	:=	.F.
	End
return (lResult)
	...
	If nOpcion == 1
		If aCodigo[1] != "" .Or. aCodigo[3] != ""
			If nApp != 2 .Or. lConfirma != Nil
				cQuery	:=	" SELECT Z2_ITEM, Z2_COSTO, Z2_COD1, Z2_DESC1, Z2_UM1, Z2_LOTE1, Z2_CANT1"
				...
				If .Not. (cDatos)->(eof())
					...
					(cDatos)->(dbCloseArea())
				Else
					vtAlert("El producto no pertenece a este pedido.", "SIGAACD", .T.)
					(cDatos)->(dbCloseArea())
					Return (lResult)
				End If
			Else
				...
				If .Not. (cDatos)->(eof())
					...
					(cDatos)->(dbCloseArea())
				Else
					vtAlert("Archivo no encontrado.", "SIGAACD", .T.)
					(cDatos)->(dbCloseArea())
					Return (lResult)
				End If
			End If
...
User Function IMX4601G(nApp, aDatos)
	...
	ElseIf	nApp == 3
		...
		SF2->(dbSkip())
	End If
	vtAlert("El inventario se actualizo correctamente.", "SIGAACD")
Return
		...
		If VTLastKey() == 13 //Enter
			If .Not. empty(cFolio)
				...
				If .Not. (cDatos)->(eof())
					...
					End If
				Else
					VTAlert(Chr(13) + Chr(10) + "No se encontro el archivo", "SIGAACD", .T.)
				End If
				(cDatos)->(dbCloseArea())
			End If
		ElseIf VTLastKey() == 27 //ESC
...
User Function IMX4601J(nApp, aDatos, nOpcion, aSZ3)
	...
	If len(aItems) <= 0
		lContinue	:=	.F.
		vtAlert("No se econtro el archivo.", "SIGAACD", .T.)
	End If
	Do While lContinue
		nPos	:=	0
		vtClear()
