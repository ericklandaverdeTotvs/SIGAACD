	...
	Do While .Not. lAcceso
		...
		If __CUSERID # "000000"
			...
			If SubSTR(aUser[3] [nHandler], 3, 1) == "X"
				VTAlert("ACCESO" + Chr(13) + Chr(10) + "RESTRINGIDO", "Modulo RF", .T.)
			Else
				lAcceso := .T.
			End If
		Else
...
User Function IMX4600A()
	...
	Do While .Not. lProcess
		...
		@02, 00 VTSay "1) Recepcion"
		@03, 00 VTSay "2) Transferencias"
		@04, 00 VTSay "3) Picking"
		@05, 00 VTSay "4) Devoluciones"
		@07, 00 VTSay "0) Cerrar sesion"
...
User Function IMX4600G(cSerie, nFolio, cDocument)
	...
	Do While allTrim(SX3->X3_ARQUIVO) == cTable
		...
		Do Case
			Case SX3->X3_TIPO == "C"
				aAdd(aValues, space(SX3->X3_TAMANHO))
			Case SX3->X3_TIPO == "N"
				aAdd(aValues, 0)
			Case SX3->X3_TIPO == "D"
				aAdd(aValues, cToD("  /  /  "))
...
User Function IMX4600I(nItem, cProducto, aDatos, nOpcion)
	...
	If nOpcion == nil
		cResult	:=	posicione("SZ2", 1, xFilial("SZ2") + aDatos[1] + str(aDatos[2], 12, 0) + str(nItem, 2, 0), "Z2_DEPOT1")
	Else
		cResult	:=	superGetMv("MV_IMX4602", .F., "01")
	End If
...
//Versión: 1
//Descripción: Listado de documentos SZ8.
User Function IMX4600L(nOpcion, cDocumento)
	Local cQuery	:=	""
	Local cDatos	:=	"IMX4600L"
	Local aDatos	:=	{}
	Local aItem 	:=	{}
	
	If nOpcion == 1
		cQuery := " SELECT DISTINCT Z8_DOC, Z8_CLIENTE, Z8_LOJA, Z8_NOMBRE"
	ElseIf nOpcion == 2
		cQuery := " SELECT Z8_DOC, Z8_CLIENTE, Z8_LOJA, Z8_NOMBRE, Z8_FECHA, Z8_COD, Z8_GRUPO, Z8_DEVPER, Z8_DEVSOL,"
		cQuery += " Z8_VENQTY, Z8_VENVAL, Z8_SOLQTY, Z8_SOLVAL, Z8_EVAL, Z8_ST, Z8_SUMDEV"
	End If
	cQuery	+=	" FROM " + retSqlName("SZ8") + " SZ8"
	cQuery	+=	" WHERE D_E_L_E_T_ = ' '"
	//cQuery	+=	" AND Z8_ST <> 'R'"
	cQuery	+=	" AND Z8_FECHLIM >= '" + dToS(dDatabase) + "'"
	If cDocumento != nil .And. .Not. empty(cDocumento)
		cQuery	+=	" AND Z8_DOC = '" + cDocumento + "'"
	End If
	If nOpcion == 1
		cQuery	+=	" ORDER BY Z8_DOC"
	End If
	dbUseArea(.T., "TOPCONN", tcGenQry(, ,cQuery), cDatos, .T., .T.)
	Do While .Not. (cDatos)->(eof())
		aItem	:=	aClone({})
		aAdd(aItem, (cDatos)->Z8_DOC)
		aAdd(aItem, (cDatos)->Z8_CLIENTE)
		aAdd(aItem, (cDatos)->Z8_LOJA)
		aAdd(aItem, (cDatos)->Z8_NOMBRE)
		If nOpcion == 2
			aAdd(aItem, cToD((cDatos)->Z8_FECHA))
			aAdd(aItem, (cDatos)->Z8_COD)
			aAdd(aItem, (cDatos)->Z8_GRUPO)
			aAdd(aItem, (cDatos)->Z8_DEVPER)
			aAdd(aItem, (cDatos)->Z8_DEVSOL)
			aAdd(aItem, (cDatos)->Z8_VENQTY)
			aAdd(aItem, (cDatos)->Z8_VENVAL)
			aAdd(aItem, (cDatos)->Z8_SOLQTY)
			aAdd(aItem, (cDatos)->Z8_SOLVAL)
			aAdd(aItem, (cDatos)->Z8_EVAL)
			aAdd(aItem, (cDatos)->Z8_ST)
			aAdd(aItem, (cDatos)->Z8_SUMDEV)
		End If
		aAdd(aDatos, aClone(aItem))
		(cDatos)->(dbSkip())
	End
